# Description: A Tic Tac Toe Game with the MiniMax Algorithm for the CPU to make the best move and the Random module for the CPU to make a random move.
# Difficulty: Normal - You can either lose, win or make a tie 
...
# Computer's Turn:
    def cpus_turn(self):
      turn = rd.random() # CPU randomly chooses a number between 0 and 1, deciding if it should make a random move or the best move based on probability

      if turn < 0.75: # 25% chance to make a random move
        ...
        if board_2[i - 1] != player_instance.symbol and board_2[i - 1] != cpu.symbol:
          board_2[i - 1] = cpu.symbol
          board_index() # Update the board_1 with the new move
        else: 
          self.cpus_turn() # Repeat if the position is already taken
